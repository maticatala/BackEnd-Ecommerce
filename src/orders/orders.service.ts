...
export class OrdersService {
  constructor(
    ...
    private readonly opRepository: Repository<OrdersProducts>,
  ...
  async create(createOrderDto: CreateOrderDto, currentUser: User) {
    try {
      ...
      const payment = paymentEntities[0];

      if (payment.paymentType === 'Mercado Pago') {
        if (
  ...
  async updatePaymentStatusMp(
    ...
    if (processingStatuses.includes(payment.status)) {
  ...
  async update(
    ...
    if (
      ...
      throw new BadRequestException(
        `The order has already been  ${order.status}`,
      );
    }
  ...
  async getLastUserOrder(currentUser: User) {
    const order = await this.orderRepository.findOne({
      where: {
